...
const SmsClient: React.FC<SmsClientProps> = ({ apiKey, user, devices, wsPush }) => {
	...
	const startLoadingTimeout = (type: 'threads' | 'messages') => {
		...
		timeoutRef.current = setTimeout(async () => {
			...
			try {
				...
				const smsLike = recent.filter(p => 
					p.type === 'sms_changed' || p.type === 'mirror'
				).filter(p => !selectedDevice || p.source_device_iden === selectedDevice);
				// Deduplicate by title+body+timestamp
				const keySet = new Set<string>();
				const deduped: Push[] = [];
				smsLike.sort((a,b) => (b.modified || b.created) - (a.modified || a.created)).forEach(p => {
					const k = `${p.title || ''}|${p.body || ''}|${p.created}`;
					if (!keySet.has(k)) { keySet.add(k); deduped.push(p); }
				});
	...
	useEffect(() => {
		if (selectedDevice) {
				...
				service.fetchSMSThreads(apiKey, user.iden, selectedDevice).then(() => {
					setUsingFallback(false);
					...
					<div className="flex flex-col h-full bg-slate-50">
							 <div className="flex items-center p-3 bg-white border-b border-slate-200 shadow-sm">
									<button 
										...
										type="button"
										onClick={() => setIsComposing(false)} className="mr-3 p-1 hover:bg-slate-100 rounded-full">
					...
					<div className="flex flex-col h-full bg-slate-50">
							<div className="flex items-center p-3 bg-white border-b border-slate-200 shadow-sm">
									<button 
										aria-label="Go back to SMS list"
										type="button"
										onClick={() => setSelectedThreadId(null)} className="mr-3 p-1 hover:bg-slate-100 rounded-full">
											<ArrowLeft className="w-5 h-5 text-slate-600" />
									</button>
									...
									{timeoutError && (
											<div className="p-4 text-center text-xs text-amber-600 bg-amber-50 rounded-lg border border-amber-100 mb-4">
													...
													<button 
														...
														type="button"
														onClick={handleRetry} className="block w-full mt-2 text-amber-700 font-bold hover:underline">Retry</button>
											</div>
									)}
									{loadingMessages ? (
	...
	return (
		<div className="flex flex-col h-full relative">
				...
				<div className="p-4 border-b border-slate-200 bg-white flex justify-between items-center shadow-sm z-10">
						...
						<button 
							...
							type="button"
							onClick={() => service.fetchSMSThreads(apiKey, user.iden, selectedDevice)} className="p-2 text-slate-500 hover:text-emerald-600 rounded-full">
		...
		{loadingThreads && threads.length === 0 ? (
					<div className="flex justify-center py-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-600"></div></div>
						...
						<div className="p-4">
							<h4 className="text-xs font-semibold text-slate-700 mb-2">Recent SMS notifications</h4>
							<div className="space-y-2">
								{fallbackNotifications.map((p, idx) => (
									<div key={p.iden || idx} className="p-3 bg-white border border-slate-200 rounded-lg">
										<div className="text-sm font-semibold text-slate-800 truncate">{p.title || 'SMS'}</div>
										<div className="text-xs text-slate-600 mt-1">{p.body}</div>
										<div className="text-[10px] text-slate-400 mt-1">{new Date((p.modified || p.created) * 1000).toLocaleString()}</div>
									</div>
								))}
							</div>
							<div className="mt-3 text-xs text-slate-500">Try the Retry button above once your phone is awake.</div>
						</div>
					) : (
						threads.map(thread => {
